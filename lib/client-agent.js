...
const { authenticateWorldStageAccount } = require('./worldstage-auth');
...
class WorldStageClientAgent extends EventEmitter {
  ...
  async authenticateAccount(mode, credentials = {}) {
    const result = await authenticateWorldStageAccount({
      mode,
      siteOrigin: this.config.siteOrigin,
    ...
    const snapshot = this.saveConfig({
      accountToken: result.authToken
    });
    return {
      snapshot,
