...
class WorldStageClientAgent extends EventEmitter {
  ...
  libraryWorkspacePath(entry = {}) {
    const manifestPath = String(entry && entry.manifestPath || '').trim();
    if (!manifestPath) return '';
    const stagingDirectory = path.dirname(path.resolve(manifestPath));
    if (path.basename(stagingDirectory) !== 'staging') return '';
    return path.dirname(stagingDirectory);
  }

  jobMatchesLibraryEntry(job = {}, entry = {}) {
    ...
    return (
      (videoId && String(job.videoId || '').trim() === videoId)
      || (localPath && String(job.localFilePath || '').trim() === localPath)
      || (manifestPath && String(job.downloadManifestPath || '').trim() === manifestPath)
    );
  }

  removeLibraryLocalArtifacts(entry = {}) {
    const localPath = String(entry.localPath || '').trim();
    const manifestPath = String(entry.manifestPath || '').trim();
    const workspacePath = this.libraryWorkspacePath(entry);
  ...
  removeLibraryItem(input = {}) {
    const entry = this.findLibraryEntry(input);
    this.removeLibraryLocalArtifacts(entry);
    this.state.jobs = this.state.jobs.filter((candidate) => {
      if (!this.jobMatchesLibraryEntry(candidate, entry)) return true;
      if (candidate.status === 'running' || candidate.status === 'queued' || candidate.status === 'blocked') {
        return true;
      }
      this.removeJobWorkspace(candidate);
      return false;
    });
  ...
  async applyRemoteCommand(command, iso = nowIso(this.clock)) {
    ...
    try {
      ...
      if (commandName === 'remove_seed' || commandName === 'remove_hosted_video') {
        this.removeLibraryItem(libraryLocator);
        await this.reportRemoteCommandResult(normalized, {
          result: 'applied',
          note: 'Seed removed and the local copy deleted.',
          occurredAtIso: iso
        }, iso);
        return;
      }
