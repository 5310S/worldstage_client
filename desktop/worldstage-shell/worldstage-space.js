...
function dispatchWorldStageFallbackReady() {
  if (canvas) {
    canvas.style.opacity = '0';
    canvas.style.pointerEvents = 'none';
  }
  if (frameStage) {
    frameStage.style.transform = 'scale(1)';
  }
...
if (!canvas || !window.WebGLRenderingContext) {
  dispatchWorldStageFallbackReady();
} else {
  ...
  try {
    ...
    console.warn('worldstage-space fallback:', error && error.message ? error.message : error);
    dispatchWorldStageFallbackReady();
  }

  if (!renderer) {
    // Stop here when WebGL is unavailable so the auth shell can still work.
  } else {
